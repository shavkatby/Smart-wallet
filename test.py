...
def text_to_int(text):
    ...
    total = 0; temp = 0
    for w in words:
        if w.isdigit(): val = int(w)
        elif w in soz_raqam: val = soz_raqam[w]
        else: continue
        if val >= 100:
            temp = (temp if temp > 0 else 1) * val
            total += temp; temp = 0
        else: temp += val
    return total + temp
...
def convert_to_som(text):
    try:
        ...
        if any(x in text for x in ["$", "dollar", "usd"]): return amount * USD_RATE
        if any(x in text for x in ["euro", "eur", "evro"]): return amount * EUR_RATE
        return amount
    except: return 0
...
async def show_stats(m: types.Message):
    ...
    msg = (f"📈 **{period_name} uchun tahlil:**\n"
           ...
           f"━━━━━━━━━━━━━━\n"
           f"💳 **Balans: {user[1]:,.0f} so'm**")
    
    # Inline tugma PDF yuklash uchun
...
async def send_period_pdf(call: types.CallbackQuery):
    ...
    os.remove(path); await call.answer()
...
async def full_pdf_report(m: types.Message):
    ...
    await m.answer_document(FSInputFile(path), caption="Sizning barcha amallaringiz hisoboti."); os.remove(path)
...
def upload():
    uid = request.args.get('uid'); status = request.args.get('status')
    if not uid: return "UID missing", 400
    temp_voice = f"v_{uid}.wav"
    try:
        with wave.open(temp_voice, 'wb') as wf:
            wf.setnchannels(1); wf.setsampwidth(2); wf.setframerate(16000); wf.writeframes(request.data)
        r = sr.Recognizer()
        with sr.AudioFile(temp_voice) as source: audio = r.record(source)
        text = r.recognize_google(audio, language='uz-UZ')
        amt = convert_to_som(text)
        if amt == 0: amt = text_to_int(text)
        if amt > 0:
            with get_db() as conn:
                ...
                conn.execute("INSERT INTO history (chat_id, type, amount, date) VALUES (?,?,?,?)", (uid, status, amt, datetime.now()))
                conn.commit()
                loop.call_soon_threadsafe(asyncio.create_task, bot.send_message(uid, f"🎙 {text}\n{'✅' if status=='kirim' else '❌'} {amt:,.0f} so'm\n💰 Balans: {new_bal:,.0f} so'm"))
